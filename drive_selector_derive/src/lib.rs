...
#[proc_macro_derive(DriveSelector)]
pub fn derive_drive_selector(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    expand_derive_drive_selector(&input).unwrap().into()
}

fn expand_derive_drive_selector(
    ...
    let field_output: Vec<proc_macro2::TokenStream> = match cont.data {
        ...
        _ => return Err("Only able to produce DriveSelector for plain Struct".to_owned()),
    };

    let ident = cont.ident;
    let (impl_generics, ty_generics, where_clause) = cont.generics.split_for_impl();
    Ok(quote! {
        impl #impl_generics drive_selector::DriveSelector for #ident #ty_generics #where_clause {
...
fn selector_for_field<'a>(field: &serdei::ast::Field<'a>) -> proc_macro2::TokenStream {
    let field_name = field.attrs.name().deserialize_name();
    let typ = field.ty;
    if field.attrs.flatten() {
        quote! {
            <#typ>::selector_with_ident(ident, selector);
        }
    } else {
        quote! {
            <#typ>::selector_with_ident(&{
                if ident.is_empty() {
                    #field_name.to_owned()
                } else {
